...
begin
  ...
  begin
    ShowMessage('O nome informado é muito longo');
    Exit;
  end;

  if length(email) > 200 then
  begin
    ShowMessage('O email informado é muito longo');
    Exit;
  end;

  if length(senha) > 60 then
  begin
    ShowMessage('A senha informada é muito longa');
    Exit;
  end;
  ...
  begin
    ShowMessage('Este nome já está sendo utilizado, por favor escolha outro');
    qryRepeatUser.Close;
    Exit;
  end;
  ...
  begin
    if Messagedlg('Nenhum dos usuários cadastrados no sistema possui permissão para conceder privilégios a outros usuários. Deseja conceder a este novo usuário esse privilégio?', mtConfirmation, [mbOk, mbCancel], 0) = MrOk then
      configUsers := true;
  end;
